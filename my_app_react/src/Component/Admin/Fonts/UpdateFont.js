...
const UpdateFont =(props)=>{
    ...
    const fontUpdate=(e)=>{
        ...
        axios.post(`/api/update_font/${props.match.params.id}`,formData).then(res=>{
    ...
    useEffect(()=>{
        axios.get(`/api/view_font/${props.match.params.id}`).then(res=>{
            if(res.data.status===200)
            {
...
if(loading)
    {
        return (
            <div className="container px-4 py-4 text-primary">
                <span className="h1 my-4">Update Font </span>
    ...
    return(
        <div className="container px-4">
            <h1 className="my-4 d-none d-md-block">Update Font</h1>
            <i className="fa fa-edit fa-3x text-primary my-4 d-md-none"></i>

            <form onSubmit={fontUpdate} className="my-5">
                ...
                <div className="row mt-4">
                    <div className="d-grid">
                        <button type="submit" className="btn btn-primary btn-block py-3 offset-md-1 col-md-10">Update Font</button>
                    </div>
                </div>
            </form>
        </div>
