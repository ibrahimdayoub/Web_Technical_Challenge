...
import Master from '../Layout/Admin/Navbar';
...
function AdminPrivateRoutes({...rest}){
    ...
    useEffect(()=>{
        axios.get('/api/authenticated_admin').then(res=>{
            if(res.status==200)
            {
        ...
        return (
            <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow sticky-top">
                <div className="container-fluid">
                    <Link className="navbar-brand" to="/admin">CGA <span className="fs-6 text-primary">Loading</span></Link>
